...
export class FingerprintEngine {
  ...
  private collectNavigator(): void {
    ...
    this.addInteger(FeatureID.HardwareConcurrency, navigator.hardwareConcurrency);
  ...
  private collectStorage(): void {
    this.addBoolean(FeatureID.LocalStorage, this.isStorageAvailable('localStorage'));
    this.addBoolean(FeatureID.SessionStorage, this.isStorageAvailable('sessionStorage'));
  ...
  private collectNetwork(): void {
    ...
    if (conn) {
      ...
      this.addString(FeatureID.ConnectionEffectiveType, conn.effectiveType ?? 'unknown');
  ...
  private collectLocale(): void {
    ...
    this.addString(FeatureID.Timezone, Intl.DateTimeFormat().resolvedOptions().timeZone);
    this.addInteger(FeatureID.TimezoneOffset, new Date().getTimezoneOffset());
    this.addString(FeatureID.DateTimeFormat, new Intl.DateTimeFormat().resolvedOptions().locale);
  }
  ...
  private collectCapabilities(): void {
    ...
    if (gl) {
      ...
      if (debugInfo) {
        this.addString(FeatureID.WebGLVendor, gl.getParameter(debugInfo.UNMASKED_VENDOR_WEBGL));
        this.addString(FeatureID.WebGLRenderer, gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL));
      }
      this.addString(FeatureID.WebGLVersion, gl.getParameter(gl.VERSION));
    }

    // Workers
    this.addBoolean(FeatureID.ServiceWorkerSupport, 'serviceWorker' in navigator);
    ...
    // CSS features
    this.addBoolean(FeatureID.CSSGridSupport, this.supportsCSS('display', 'grid'));
    this.addBoolean(FeatureID.CSSFlexboxSupport, this.supportsCSS('display', 'flex'));
  }
  ...
  addBoolean(id: FeatureID, value: boolean): void {
    ...
    if (code !== ErrorCode.Success) throw new Error(`add_boolean failed: ${this.getError()}`);
  }

  addInteger(id: FeatureID, value: number): void {
    const code = this.exports.fingerprint_add_integer(id, BigInt(value));
    if (code !== ErrorCode.Success) throw new Error(`add_integer failed: ${this.getError()}`);
  }

  addFloat(id: FeatureID, value: number): void {
    const code = this.exports.fingerprint_add_float(id, value);
    if (code !== ErrorCode.Success) throw new Error(`add_float failed: ${this.getError()}`);
  }
  ...
  addString(id: FeatureID, value: string): void {
    ...
    if (code !== ErrorCode.Success) throw new Error(`add_string failed: ${this.getError()}`);
  }
  ...
  addBytes(id: FeatureID, value: Uint8Array): void {
    ...
    if (code !== ErrorCode.Success) throw new Error(`add_bytes failed: ${this.getError()}`);
  }

...
// story: m6-registry-lock
// The FeatureID name↔value mapping is part of the wire ABI and changes only on
// a schema bump; an id is reclaimed only on a breaking schema bump. This anchor
// table is the renumbering guard — silent name→value drift becomes a compile
// error.

const feature_id_anchors = .{
    .{ "UserAgent", 0 },
    .{ "Language", 1 },
    .{ "Languages", 2 },
    .{ "Platform", 3 },
...
comptime {
    for (feature_id_anchors) |anchor| {
        const got = @intFromEnum(@field(FeatureID, anchor[0]));
        if (got != anchor[1]) {
            @compileError(std.fmt.comptimePrint(
                "FeatureID renumbering guard: '{s}' is {d}, expected {d}",
                .{ anchor[0], got, anchor[1] },
            ));
        }
    }
    std.debug.assert(@intFromEnum(FeatureID.Count) == 102);
}

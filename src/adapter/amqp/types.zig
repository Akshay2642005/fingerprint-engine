...
pub const QueueBindOptions = struct {
    /// Name of the queue to bind.
    queue: []const u8,
    /// Name of the exchange to bind to.
    exchange: []const u8,
    /// Routing key for the binding (e.g. `result.*` to receive every result).
    routing_key: []const u8,
    /// Do not await a binding confirmation.
    no_wait: bool,
};

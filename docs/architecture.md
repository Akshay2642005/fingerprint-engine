...
## Layer 3: Browser SDK (`src/clients/browser/`) and WASM (`src/wasm/`)

The browser SDK lives in `src/clients/browser/`: hand-written TypeScript
signal collectors in `src/`, the demo page in `demo/`, and the npm
`package.json`. The `dist/` bundle is generated by Zig.

The WebAssembly module lives in `src/wasm/` — an infra artifact

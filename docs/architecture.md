...
## Layer 3: Browser SDK (`src/clients/browser/`) and WASM (`src/wasm.zig`)

The browser SDK lives in `src/clients/browser/`: hand-written TypeScript
signal collectors in `src/` and the npm `package.json`. The `dist/` bundle is
generated by Zig. The npm payload is strictly the runtime SDK — the demo page
lives outside the package at `examples/demo.html` (served from the repo for
development, never shipped).

The WebAssembly module lives at `src/wasm.zig` — an infra artifact
(ReleaseSmall) that the package generator inlines into the bundle and that
serves the benchmark harness and wasmtime test containers.

```
src/clients/browser/
├── src/                # hand-written TypeScript SDK (collectors/, bindings/)
├── scripts/            # build-time templates
└── package.json        # npm metadata; dist/ generated by `zig build clients:browser`

examples/
└── demo.html           # dev-only demo page (never shipped in the npm package)

src/wasm.zig            # Zig WASM exports (ReleaseSmall)
```

`zig build clients:browser` derives `FeatureID`/`FeatureType` from the Zig
model (single source of truth) and injects the package version into the
bundle.

...
- Multi-stage `deploy/Dockerfile.worker` — static worker binary on `scratch`, exposed on 8080 — plus a CI `worker-build` job that builds the executable and uploads it as an artifact.

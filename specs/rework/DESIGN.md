...
Browser
  ...
  → transport.ts: POST to ingress endpoint
  → ingress → RabbitMQ → workers
```
...
src/
  ...
  transport.ts      POST SignalPackage bytes to ingress (configurable URL)
index.d.ts          hand-written declarations
...
flowchart TD
    ...
    P --> I[Ingress Service]
    I --> Q[(RabbitMQ)]
    Q --> WK[Workers validate normalize hash entropy risk]
    WK --> E[Engine.process]
    E --> FC[FingerprintComputed RiskResult SimilarityResult]
    FC --> FP[Fraud Platform Go - separate repo]
...
| `tests/clients/browser/` (TS) | package.ts serializer golden vectors vs Zig fixtures; SDK type-check; e2e against a fixture ingress |
